...
本册按三拍走：**看 → 懂 → 判与望**。每一拍是一段主题弧，不是逐章标签；主线靠三拍推进——第 1–7 章构成第一、二拍与第三拍的"判"，第 8 章是第三拍的"望"（回望·选读，跳过不影响主线）。

**第一拍，看：现象与动力学（第1–3章）。** 训练在容量、景观、时间三条轴上都不单调——双下降、梯度下降总能落进泛化盆地、grokking/涌现的"突然"。三条轴表面不同，背后是同一个故事：**表示是慢变量，读出是快变量**；训练真正在做的，是把网络内部表示一步步重构。这一拍攒下的反常（为什么越大越好、为什么突然开窍），就是后两拍要回答的问题。

**第二拍，懂：规律与机制（第4–6章）。** 先给规律、再拆机制（拍内顺序不变）：第 4 章先兑现第一条能外推的规律——尺度定律——并摆出三个起源候选（数据谱／表示压缩与叠加／语言结构随机图）；随后两章逐候选验收——第 5 章用谱工具与特征学习接住"数据谱"（lazy/rich、μP 超参迁移），第 6 章用表示结构接住"压缩/叠加"（叠加几何、SAE 可恢复、induction 电路）；"语言结构（随机图）"仍停在 §4.3 与参考文献，本书不展开。

**第三拍，判：边界怎么量、何时失效（第7章）。** 怎么衡量一条规律、它何时失效，给出五类边界。没有失效条件的规律不是规律，是口号。第 7 章全章即工具箱：测量方法直接可用，§7.4–7.6 为选读与方法论。

**第三拍，望：理论之梯（第8章，回望·选读）。** 判而未果处，才需要望——第 7 章的"墙"正是第 8 章回望的理由。第 8 章把不可能性换一只手看：经典/现代理论按四路给 AI 供水（词汇／算法／结构／护栏）、滞后-复用是常态、现代理论在证伪中重锚定。主线上读完第 7 章后按需回望，跳过不影响主线。第 8 章为回望视角章：十余条理论的贡献清单本身即查阅工具。
...
> **本章路线**：§1.0–§1.2 先问“误差从哪来、被怎么分解”；§1.3–§1.4 看经典理论及其失效；§1.5–§1.7 看双下降与两速度、隐式正则化机制；§1.8–§1.10 收束。
...
双下降解释了"测试误差为什么非单调"，但没有解释"第二次下降为什么恰好降得好"。Chou 等人的工作给出一个统一视角：**两速度分解**（arXiv:2605.27078，Flatiron/Harvard/Kempner 团队，预印本）。
...
1. **最大间隔偏向**：在可分数据上，梯度下降（包括带标签噪声的训练）收敛到的解，方向趋近最大间隔分类器。隐式正则化的效果，可以写成显式的几何目标（Soudry 等 2018 起的系列工作）。
...
> **本章路线**：§2.0–§2.1 看高维景观（鞍点为主）与逃逸；§2.3–§2.5 看解的盆地、edge of stability 与守恒慢变量；§2.6 收束到隐式正则化动力学；§2.7–§2.9 落地、误解与回顾。
...
高维景观的第一个事实是：**驻点里，极小值指数级稀少，鞍点是常态**。粗算如下：把每个方向当作独立的随机符号，向上弯记为 $+$，向下弯记为 $-$。局部极小要求 $d$ 个方向全部为 +，概率约 $(1/2)^d$。$d = 10$ 时约千分之一，$d = 100$ 时约 $10^{-30}$。真实 Hessian 的方向彼此相关，具体数值不同，但量级上的判断一致：维度越高，极小值越罕见（随机场理论的严格版本见 Bray & Dean 2007：大多数临界点都是鞍点）。
...
为什么锐度会停在 $2/\eta$ 附近？分两层讲。**第一层：损失下降为什么推高锐度。** 网络为了拟合数据，决策边界会越用越锐利，损失曲面的曲率随之上升。这个"渐进锐化"（progressive sharpening）在深度线性网络里有严格证明（Even 等 2023），在真实网络里是普遍观察。它把锐度一路推高，直到触到 $2/\eta$ 的稳定性边界。**第二层：越过边界后为什么被压回来。** 锐度超过 $2/\eta$ 后，参数在锐利方向上每一步都过冲，进入确定性的来回震荡。震荡的平均效果，相当于给损失加了一项曲率惩罚：三阶曲率决定震荡偏向哪一侧，结果把参数的平均位置推向曲率更低处（Damian 等 2022 给出三阶项驱动的平衡）。Cohen 等 2025 把两层合并成一句话：edge of stability 上的整条轨迹，等价于一条"带曲率惩罚的梯度流"。两个力——拟合推高曲率、震荡压回曲率——在 $2/\eta$ 附近达到动态平衡。
...
大奇异值方向先学到目标（50 步即达 2.677），小奇异值方向慢慢跟上（600 步才到 0.948）。这就是第 1.7 节低秩偏向的动力学版本：**奇异值按大小顺序被学习**（Saxe 等 2014 在输入白化设定下的精确结果）。

这个顺序来自参数化的对称性，机制可以拆成三步。**第一步：对称性制造守恒量。** 网络 $W_2 W_1$ 有一个重标度对称性：$W_1 \to c W_1$、$W_2 \to c^{-1} W_2$ 不改变输出。损失沿这个方向完全不变，梯度为零，所以梯度流在这个方向上的某个组合量守恒。这正是诺特定理（Noether's theorem）说的"对称性对应守恒量"，Kunin 等 2021 把它系统化。**第二步：守恒量把能量交换固定住。** 能量在这里指两层权重的范数（奇异值平方和）。重标度对称性意味着：把 $W_1$ 放大、把 $W_2$ 缩小，输出与损失都不变，梯度为零——所以两层之间的范数差被冻结，能量不能自由地在两层之间流动。若没有这个冻结，模式会借着重标度自由度互相串扰；冻结之后，配合白化输入的关键假设（模式间的交叉项消失，Saxe 2014），每个奇异值方向才能各自独立演化。**第三步：解耦后按大小排序。** 白化输入下，Saxe 等 2014 证明每个奇异值满足独立的演化方程。小奇异值阶段，增速近似正比于自身大小（$d\sigma_i/dt \approx \sigma_i \cdot c_i$）。初始时所有 $\sigma_i$ 都很小，大奇异值先起飞，小奇异值在后面慢慢追。三步合起来：对称性 → 守恒量 → 解耦 → 顺序学习。
...
> **图5**：奇异值顺序学习——有效矩阵的奇异值随训练步数逐级追上最终值（Saxe 2014）。
...
**线性缩放律**。把学习率与批大小同时放大相同的倍数，并把训练步数按比例缩小（保持看过的样本数不变），训练轨迹近似不变（Goyal 等 2017）。这是一条被实践反复验证的经验律：在批大小从 256 调到 1024 时，学习率从 0.1 调到 0.4，效果基本持平。这条经验律有一个边界：**临界批大小**（critical batch size，记作 $B_{\text{crit}}$，下标 crit 即 critical；McCandlish 等 2018）。它的模型很干净，两条公式：串行时间 $S(B) = S_{\min}(1 + B_{\text{crit}}/B)$，总算力 $C(B) = C_{\min}(1 + B/B_{\text{crit}})$。批大小小于临界值时，加大批大小能等比例省时间；超过后，省时间的收益越来越小，算力却线性上涨。给一个示意数字：设 $B_{\text{crit}} = 2000$。批大小从 512 提到 2000 再提到 8000，串行时间约为最小值的 4.9 → 2.0 → 1.25 倍；总算力约为 1.3 → 2.0 → 5.0 倍。把批从 2000 提到 8000，时间只省 0.75 个单位，算力多烧 3 个单位——这笔交易通常不划算。McCandlish 等实测常见任务，$B_{\text{crit}}$ 多在 $10^2$ 到 $10^4$ 量级，具体数值随任务与模型变化。
...
> **本章路线**：§3.0–§3.1 看 grokking 现象；§3.2 给出相变语言；§3.3–§3.5 看五种理论、涌现之争与平坦性；§3.6–§3.8 收束。
...
一句话记住三个特征：**先记住、再停滞、最后"突然开窍"**。（Transformer 原设定里跃迁更陡峭，Power 等 2022；本演示用 MLP，跃迁相对平缓，但"训练饱和→测试滞后→快速追赶"的结构一致。）
...
|:--|:--|:--|:--|:--|
| ① 可证明理论（To Grok Grokking，arXiv:2601.19791） | 岭回归 + 权重衰减下，grokking 三阶段可端到端证明：先过拟合、再泛化迟滞、最后开窍 | 可证明的收敛分析 | 过参数化线性回归（权衰驱动） | ICML 2026 Oral，作者 Vardi/Safran 为知名理论研究者；线性设定，非线性网络尚未覆盖 |
...
- Lubana 等 2025 提出第三种图景：能力确实可以突现，但它是潜空间中某个子结构连续变化、跨过阈值时被激活的结果。这种**渗流**（percolation）式相变见 arXiv:2405.12617。
...
**Neural collapse**（神经坍缩，Papyan 等 2020）是一个被广泛复现的经验规律：训练结束时，同一类的样本表示聚拢到类均值附近，不同类的均值排成正交的单纯形结构。**它不保证泛化**。

证据来自 grokking 的过程（NeurIPS 2025）：在泛化跃迁之前，模型已经记住训练集，表示也接近 collapse，但测试准确率仍然趴在随机水平。真正与泛化同步出现的，是解的平坦性（第 2.4 节的锐度下降）。换句话说：collapse 是"记住得好"的样子，平坦性是"泛化得好"的原因；**平坦性必要，neural collapse 非必要**。grokking 把两者在时间上分开了。
...
2. **长平台期别急着早停**。grokking 的平台期可能长达训练全程的一半以上（本演示：训练 2000 步饱和，测试 7000 步才接近满分）。验证损失长期不降时，先确认表示是否仍在更新（第 3.1 节的表示变化率），再决定是继续等还是调参——直接早停会杀死即将到来的泛化。LLM 预训练里"记忆→泛化"的相变迹象已有报告（Where to Find Grokking in LLM Pretraining，arXiv:2506.21551）。规模效应与玩具设定是否同源，仍有争议。
3. **权重衰减是 grokking 的开关之一**。本演示与可证明理论（①）都用权重衰减/岭回归驱动泛化跃迁；没有它，模型更容易停留在"死记硬背"的解上。调参时把权重衰减当成"泛化旋钮"而非"防过拟合补丁"。

## 3.7 常见误解

**误解一："grokking 只是玩具模型的现象。"** grokking 的本质——训练饱和后泛化延迟——在大模型预训练里也有对应迹象（记忆早于泛化、指标长时间停滞）。但证据分级是"迹象"而非"实锤"（arXiv:2506.21551 及其后续讨论）。玩具模型的价值在于把机制隔离出来；机制是否同源，是开放问题，不是否定它的理由。
...
> **本章路线**：§4.0–§4.1 看幂律现象；§4.2 讲清“能外推才是定律”；§4.3–§4.5 看起源三候选、最优配比与失效边界；§4.6–§4.8 收束。
...
这条经验律在真实 LLM 上被反复验证（Kaplan 2020、Chinchilla 2022 的曲线；现象层面《基座模型》§2.3 已述，本章不重复数字，只接机制与边界）。它对实践意味着：**损失不是"加一点规模降一点"，而是按一条可估计的幂律在降**。
...
**数字演示⑦** 用的就是慢尾巴谱（Sharma & Kaplan 可解模型的思想，arXiv:2210.16859；主流假说）。教师是一个 4096 维线性函数，权重按 $w_k \propto k^{-0.8}$ 衰减，能量谱 $w_k^2 \propto k^{-1.6}$。这个衰减不慢不快，正好在后面的指数上留下可测的痕迹。学生是取前 $p$ 个坐标的线性回归，$p$ 即"参数量"；噪声方差 $0.09$ 即不可约损失 $L_\infty$。数据量固定为 20000，让参数轴的变化不被采样噪声淹没。训练后测测试损失：
...
三种候选不互相排斥：它们回答的是"哪一个稀缺资源在决定斜率"——数据的信息谱、表示的容量、语言的结构。这和第 3.4 节涌现之争是同一方法论：**你盯哪个量，就看到哪个起源**。统一的框架想把多轴合成一个超曲面（UNSL：Unified Neural Scaling Laws，arXiv:2605.26248；前沿未定）。它把参数、数据、算力、时间都当作耦合的自变量。
...
**第一，不可约损失的饱和**。$L_\infty$（数据里的不可压信息）把幂律摁在地上：规模再大，$L(N) - L_\infty$ 趋近 0，曲线在双对数上弯向水平。在这个饱和区，外推会高估"加规模的收益"。信息论给出它的下界：长上下文建模的互信息缩放定律（L2M，NeurIPS 2025，MIT，arXiv:2503.04725）说明，损失底由数据的信息量决定，不归模型管。实践中，这就是"数据墙"和"模型崩塌"（Shumailov 2024）讨论的背景——$L_\infty$ 定住了你能买到的下限。

**第二，相变区不幂律**。第 3 章的 grokking 与涌现、第 1 章的双下降，都是"损失不随规模单调滑"的区域。在这些区域外推，等于用一条直线去预言一次跳变——必然落空。

**第三，测试时算力的收益分层**。测试时算力（test-time compute，TTC）也有自己的缩放定律。它甚至有可证明版本：Provable Scaling Laws for the Test-Time Compute of LLMs（NeurIPS 2025；主流假说）。用到它的设定，见《基座模型》§3.3；数学工具在《AI数学》第 7 章。但 TTC 不是简单幂律：收益随任务难度与验证器状态变化（Kinetics，arXiv:2506.05333，NeurIPS 2025；主流假说）。验证器一不完美，增益就被钝化（ROC-n-reroll，MPI，主流假说）。TTC 是"要做更多计算"的轴，但它服从的是有条件的、分层的规律，不是一条外推到底的直线。
...
> **本章路线**：§5.0–§5.1 看 lazy/rich 之分；§5.2 讲清宽度与参数化怎样锁住特征；§5.3 给出 μP 与最优学习率不变量；§5.4 标出参数化理论的坑；§5.5–§5.7 收束。
...
**NTK 极限**。神经正切核（neural tangent kernel，NTK，Jacot 等 2018）回答"什么条件下特征会锁死"。答案是：当宽度趋于无穷、参数在训练中只动一点点时，训练过程等价于在固定特征上做核回归。回归的核，是初始化瞬间的梯度方向矩阵。这个极限是严格的数学结论，也是"lazy"的精确版本：参数动得越来越小，特征根本没有机会改。
...
**如何打破锁定**。文献给出两条路。第一条是**初始化尺度**（第 5.1 节的 $\sigma$）：小初始化让特征有机会大幅重排。第二条是**参数化**——$\mu$P（maximal update parameterization，最大化更新参数化；第 5.3 节详述）让特征学习在任意宽度都被保留。理论载体也在补位。深度线性网络的精确动力学，给出了 lazy→rich 转变。严格刻画见 From Lazy to Rich: Exact Learning Dynamics in Deep Linear Networks（ICLR 2025；主流假说）。同套框架把特征学习解释成"神经低次滤波"：网络先学数据里的低阶结构，再逐层升级。这个谱理论见 Neural Low-Degree Filtering（arXiv:2605.13612；前沿未定）。这正是第 2.5 节奇异值顺序学习的特征版：大奇异值先被学到，大尺度结构先出现在表示里。

> **诚实边界**：NTK 极限本身是严格数学，但真实网络的有限宽度介于 lazy 与 rich 之间。"lazy/rich"是连续谱的两端，不是开关（Beyond the Lazy-Rich Dichotomy，arXiv:2503.18114）。另外，RNN 与 DNN 的特征学习已有统一理论（arXiv:2602.15593；前沿未定）。
...
**$\mu$P 是什么**。$\mu$P（maximal update parameterization，最大化更新参数化；Yang 2021–2023）是一套**标度规则**：回答"每一层参数的更新应该多大"。设计目标只有一个——**让特征学习不随宽度消失**。构造只有两个要点：
...
$\mu$P 的构造原则是**最大化更新**（maximal update）：让每个参数的更新对网络输出贡献同量级（$O(1)$）。这样特征层与读出层各按自己的宽度标度演化，不互相淹没。Tensor Programs 框架（arXiv:2203.03466）把这个原则严格化：保持更新的尺度守恒，就得到学习率在宽度维度的不变量。2026 年的可证明分离结果（ICML 2025）确认 $\mu$P 各尺度在无限宽下确实分离——证据分级：主流假说。超参迁移本身是工程反复验证的**共识**（综述 arXiv:2601.01306）：在 $\mu$P 下，小模型的完整超参配方能端到端迁移到大模型。
...
**embedding-layer hole**。$\mu$P 不是免检证。一项新研究发现：$\mu$P 的超参迁移在 **embedding 层**露出一个洞（Kalra 和 Barkeshli 等，arXiv:2605.21486）。这要求 embedding 的学习率远大于理论值、单独校正，否则迁移失真。证据分级：前沿未定（单篇预印本、新发现，可能被修正）。
...
> **本章路线**：§6.0–§6.1 看特征几何与叠加；§6.2 看 SAE 能恢复什么；§6.3–§6.4 拆机制（叠加与 induction 电路）；§6.5 问“什么算解释”；§6.6–§6.8 收束。
...
新的表示理论把叠加几何的来源接上了数据统计。结论一句话：特征的相关性与出现频率，决定方向如何摆放——见 How Correlations Shape Superposition（ICLR 2026；主流假说）。不相关、高频的特征更容易被做成独立方向；相关、低频的更容易挤在一起。这把第 6.1 节的"维度够不够"升级成"数据说了算"：同样的维度，不同的数据统计会养出不同的叠加几何。

叠加还是第 4 章幂律起源的一个候选（表示压缩/叠加使缩放稳健，NeurIPS 2025；主流假说）。同一块几何，一头连着能否恢复（本章），一头连着损失怎么随规模降（第 4 章）。两条线索互相印证：表示不是一笔糊涂账，而是可以用几何和稀疏性审计的。
...
真实 transformer 比这个构造复杂，但结构同源。一条 induction head 通常由两层合作：第一层先做出"上一个 token"或 bigram 表示，第二层用匹配-复制完成预测。而且电路是从训练动力中长出来的。ICL 在训练中出现的行为，已有专门的动力学刻画（Learning dynamics of emergent ICL，UCL；前沿未定）。无梯度更新时的隐式 ICL 有另一支动力学研究（arXiv:2507.16003；前沿未定）。可解的一面也有：线性注意力的 ICL 已有渐近理论（PNAS 2025，arXiv:2405.11751；主流假说），是第一个能完全算清楚的 ICL 设定。
...
1. **可复现**：同一个特征/电路，在独立的模型或数据上再出现，才算数。SAE 的可复现审计把"找特征"变成了可跑的流程（arXiv:2607.12166）。它问的不是"这个单元激活了吗"，而是"这个特征在另一份数据与模型里还在吗"。
2. **因果可验证**：光有相关不够。解释应当意味着"如果我改变这个特征，模型行为按预期变"。把特征替换/注入后看输出是否按预测的方向变，是当前区分"解释"与"巧合相关"的主要实验。
3. **知道工具的失效条件**。SAE 在特征太密集、太相关、或字典不唯一时，会给出"能重构但错位"的分解；这类失效已有专门的理论刻画（Limits of SAE，ICLR 2026；主流假说）。多义干扰本身也可以跨模型预测与迁移（Polysemantic Interference，ICLR 2026；前沿未定）。也就是说，一个"解释"即使内部自洽，也可能是另一个机制的表象。
...
> **本章路线**：§7.0–§7.2 把评估做成可靠仪表（含演示⑬）；§7.3 给出五类边界类型学；§7.4–§7.6 看对齐极限、世界模型与方法论（选读）；§7.7 回顾。
...
**第一支柱：条目级数据（item-level benchmark data）**。评测必须落在"每一条"上，而不是"总体正确率"。条目级支持复现与诊断：哪一类样本错了、错在哪一技能，都可以精确指认（Item-level Benchmark，arXiv:2604.03244；方向共识）。这是测量的最小单位。
...
**第三支柱：可预测尺度**。好的评估分数本身要有解释力与预测力：你能用小规模评估外推大规模表现（General scales，Nature 2026；方向共识、方案未定）。这正是第 4 章幂律外推在"评估"轴上的延续——评估分数和损失一样，可以是规模的函数。
...
|:--|:--|:--|:--|:--|
| 表达力 | 可计算性 | 图灵完备（2506.12027 正向结果）；无 CoT 限 TC$^0$（2402.12875） | 表达力不是瓶颈；瓶颈在串行深度 | CoT / 外部工具扩大小 |
...
| 数据与评估 | 信息论/实证 | 数据墙、不可约损失、模型崩塌、基准饱和 | 规模收益递减、分数到顶 | 合成数据 + 真实锚点 |

每一类一行话。**可计算性**：表达力不缺（图灵完备），缺的是足够深的串行步骤，所以 CoT 与工具是正解。**可学习性**：有不相容的标签，学不会就不可能；把"幻觉"想成样本复杂度的下界，结论是"别只靠训练根除幻觉"，配外接核查。**可验证性**：做严格验证是 NP-hard，但这不否定近似验证与运行时监控的价值，只是"数学保证"的上限。**可对齐性**：对齐判定问题不可判定（选读节细讲），对应的实践是"显式约束 + 停机"，而不是指望"更好的 RL"一次到位。**数据与评估**：数据墙与基准饱和把"规模与分数的收益"摁住了。顶部饱和已出现"模型得分 92% 而人类专家约 65%"的反直觉观测（2602.16763、2607.01254）。
...
**对齐有信息论上限。** 在有限的 KL 预算（允许偏离基座多远）下，奖励提升有一个上限。这个结论来自 Theoretical Limits of Language Model Alignment（Paes 等，arXiv:2605.07105；选读候选）。"更多 RL"买不来线性更高的奖励；偏离越多，回报越被稀释。这把对齐从"工程手感"推向"预算-回报的规律"，与前几章"可预测规律"的姿态一致。
...
- **主张**：LLM 是世界模型的一种特例（From Tokens to States，2606.28127；注记级，单作者观点文）。"特例"是不是真命题，取决于能否验证，不止于主张。
- **可检验的路径**：世界模型要在"路径空间"里做预测——给定当前状态与一个动作，能预测后续状态，包括不可逆分支。这个判据与本书主线一致：能不能预测，是它有没有规律的试金石（A Path-Space Formulation，2606.28751；前沿未定）。
- **机构共识**：世界模型被列为 AGI 共识方向（智源 2026 十大趋势；机构报告，前沿未定）。但"方向共识"不等于"机制确证"——这与第 4.3 节起源之争、第 6.5 节审计是同一纪律：别拿趋势当定理。
...
把全书的证据链摆出来。**现象可复现**：第 1–3 章（偏差-方差、双下降、grokking）。**机制可解**：第 2/5/6 章（优化动力学、特征学习、叠加与电路）。**规律可预测**：第 4 章（幂律与外推）。**边界可标定**：本章的五类上限。本书的立场是"规律图鉴先行，理论在后"：不宣称已经有了一门完整的深度学习科学，但每条规律都有证据分级、标得出失效条件，已经够当一张能用的图。要把它收敛成真理论，是下个阶段的事（方向见 There Will Be a Scientific Theory of Deep Learning，arXiv:2604.21691；仓库有 PDF）。
...
> **本章路线**：按“四路供水”读——§8.1–§8.3 词汇/算法/结构，§8.4–§8.9 表达、泛化、环境、因果、物理、认知；§8.10–§8.11 反哺回路与现代理论的辩证作用。各节均可独立查阅。
...
- **低秩参数化（内在维度，Aghajanyan 等 2020）**：预训练模型微调所需的更新落在低维子空间——这是 LoRA 这类"仅低秩增量"方法的真正来源；条件梯度（Frank-Wolfe）的"廉价约束步"只是直觉相似，不是直接来源。
...
- **压缩感知与稀疏恢复（Donoho 2006；Candès 等 2006）**：欠定系统里，稀疏+随机测量可精确恢复；第 6 章 SAE 恢复叠加特征与之同属"稀疏性换可恢复性"的思路——但这是可类比，不是定理继承：SAE 能否恢复取决于第 6.5 节的稀疏假设（演示⑪："够稀疏就够恢复"）。
...
- **算法对齐（Xu 等 2020；GNN）**：把"网络内部能否模仿已知算法"形式化，指导可推广架构。
...
- **现代泛化理论（2018–2020s）**：良性过拟合（Bartlett 等 2020 最小二乘插值）、双下降的统一数学（第 1.5 节）、隐式正则（梯度下降选低范数/最大间隔解）——把"VC 预言错"重释为"容量轴上的相变"，而不是丢理论。
...
- **不变性原理（IRM，Arjovsky 等 2019）**：跨环境不变的表示=稳健特征——这是 OOD 泛化的**方向共识**（主流假说，其可测性与非线性适用条件仍在检验），与第 4 章"结构可分解、可恢复"、第 5 章"特征学没学"接上。
- **因果表示学习（Schölkopf 等 2021）**：把潜变量维度解释成原因而非特征——第 6 章叠加/SAE 恢复的方向，因果版本多问"哪个是原因、哪个是效果"。
...
- **能量模型 / Hopfield（1982）**：联想记忆的吸引子动力学；**现代 Hopfield 网络=attention（Ramsauer 等 2020）**——从能量函数/吸引子出发能严格恢复注意力更新式（数学同构，主流假说），是"1980 年代物理模型变成 2017 年架构"的绝佳例子。
...
- **AI → 理论**：
  - **AlphaTensor（Fawzi 等，Nature 2022）**：强化学习在矩阵乘法算法空间搜索出更快的算法（部分方案可直接在 GPU/TPU 上运行，落地仍在探索）——其意义首先是"AI 能发现新算法"，并沿"理论（Strassen 谱系）→ AI（算力）→ 新算法 → 更快 AI"形成闭环愿景。
...
**§1 泛化与双下降**
- 双下降现象（Belkin 2019；经典）——§1.5，共识（现象）。
- 随机标签拟合（Zhang 2017）——§1.4，共识。
- Two Speeds of Learning（Chou 等，arXiv:2605.27078）——§1.6、§3，前沿未定（预印本，分量较高）。
- 模式连通（Garipov 2018、Draxler 2018）——§2.3，主流。
- 隐式正则化/最大间隔（Soudry 等 2018）——§1.7，主流假说。

**§2 优化动力学**
- 奇异值顺序学习（Saxe 2014）——§2.5，主流（白化设定精确）。
- 守恒量（Kunin 2021）——§2.5，主流。
- 渐进锐化与 EoS（Even 2023、Damian 2022、Cohen 2025）——§2.4，主流。
- 线性缩放律（Goyal 2017）、临界批大小（McCandlish 2018）——§2.6，共识。
- 随机场/鞍点（Bray & Dean 2007）——§2.1，主流（高维极小值罕见）。

**§3 grokking 与相变**
- To Grok Grokking（Xu/Vardi/Safran，arXiv:2601.19791，ICML 2026 Oral）——§3.3，主流假说。
- Grokking Nucleation Theorem（Zenodo 21223240，2026-07）——§3.2，前沿未定。★
- 奇点学习理论（Watanabe 框架，SLT）——§3.3，主流假说（成熟数学，应用是近年）。
- 玻璃弛豫 Is Grokking a Computational Glass Relaxation?（NeurIPS 2025）——§3.3，前沿未定。
- 渗流式涌现（Lubana 2025，arXiv:2405.12617）——§3.4，前沿未定。
- Where to Find Grokking in LLM Pretraining（arXiv:2506.21551）——§3.6，前沿未定（迹象级）。
- grokking 命名（Power 等 2022，*Grokking: Generalization Beyond Overfitting on Small Algorithmic Datasets*）——§3.1，奠基。
- 涌现度量依赖（Wei 等 2022 二值指标 vs Schaeffer 等 2023 连续指标）——§3.4，主流/前沿未定。
- 神经坍缩（Papyan 等 2020）——§3.5，主流（现象级）。

**§5 特征学习与 $\mu$P**
- 神经正切核（Jacot 2018，NTK）——§5.2，严格数学（无限宽极限）。
- Lazy Training（Chizat 2019）——§5.1/4.2，主流。
- From Lazy to Rich（ICLR 2025 深线性）——§5.2，主流假说。
- Neural Low-Degree Filtering（arXiv:2605.13612）——§5.2，前沿未定。
- Beyond Lazy-Rich（arXiv:2503.18114）；统一 RNN/DNN（arXiv:2602.15593）——§5.2，前沿未定。
- Tensor Programs V（Yang 等，arXiv:2203.03466）——§5.3，主流。
- $\mu$P 各尺度可证明分离（ICML 2025）——§5.3，主流假说。
- Hyperparameter Transfer 综述（arXiv:2601.01306）——§5.3，共识。
- $\mu$P embedding-layer hole（Kalra & Barkeshli，arXiv:2605.21486）——§5.4，前沿未定。★

**§4 尺度定律**
- Kaplan 2020、Chinchilla 2022——§4.1，共识（现象；《基座》§2.3 已述）。
- Sharma & Kaplan 可解模型（arXiv:2210.16859）——§4.2，主流假说（演示⑦ 载体）。
- Superposition Yields Robust Neural Scaling（NeurIPS 2025）——§4.3，主流假说。
- 神经缩放的随机图起源（Barkeshli 等，arXiv:2601.10684，ICML 2026）——§4.3，前沿未定。
- UNSL（Caballero，Mila+DeepMind，arXiv:2605.26248）——§4.3，前沿未定。★
- Practical Scaling Laws（Bryant-Liu）——§4.4，共识（方向）。
- Predictable Scaling Laws of Optimal Hyperparams（Zhou-Diao）——§4.4，主流假说。
- Provable TTC Scaling（NeurIPS 2025）——§4.5，主流假说。
- Kinetics（arXiv:2506.05333）、ROC-n-reroll（MPI）——§4.5，主流假说。
- L2M 互信息缩放（arXiv:2503.04725，MIT，NeurIPS 2025）——§4.5，主流假说。
- 模型崩塌（Shumailov 2024）、数据墙（Epoch AI）——§4.5，共识方向。

**§6 表示与机制**
- How Correlations Shape Superposition（ICLR 2026）——§6.3，主流假说。
- Limits of SAE（ICLR 2026）——§6.5，主流假说。
- Polysemantic Interference（ICLR 2026）——§6.5，前沿未定。
- 可复现审计（A Reproducible Audit of SAE Features，arXiv:2607.12166）——§6.5，主流假说。
- 线性注意力 ICL 渐近理论（arXiv:2405.11751，PNAS 2025）——§6.4，主流假说。
- Emergent ICL 动力学（UCL 10222775）、隐式 ICL 动力学（arXiv:2507.16003）——§6.4，前沿未定。
- Distinct mechanisms underlying ICL（Gibson-Cui）——§6.4，主流假说。

**§7 衡量与边界**
- Item-level Benchmark（arXiv:2604.03244）——§7.1，方向共识。
- From Benchmarks to Skills（TACL 低秩分解）——§7.1/7.2，主流假说（演示⑬ 载体）。
- General scales（Nature 2026）——§7.1，方向共识、方案未定。★（proof 版）
- 图灵完备（arXiv:2506.12027）、CoT 表达力（arXiv:2402.12875）——§7.3，正向结果。
- 不可幻觉无免费午餐（arXiv:2410.19217，ICLR 2025）；模型证伪极限（arXiv:2502.06765）——§7.3，主流（定理层）。
- NN 验证 NP-hard（Katz 2017 起）；MPNN 不可验证——§7.3，定理。
- 内对齐不可判定（arXiv:2408.08995；Scientific Reports 2025 跟进）——§7.3，定理。
- Benchmark 饱和（arXiv:2602.16763）；Benchmark Ceiling（arXiv:2607.01254）——§7.3，实证。
- Theoretical Limits of LM Alignment（arXiv:2605.07105）——§7.4，选读候选。
- When Best-of-N is Worse——§7.4，主流假说。
- Take Goodhart Seriously（arXiv:2510.02840）——§7.4，注记级。
- From Tokens to States（arXiv:2606.28127）；Path-Space 预测（arXiv:2606.28751）——§7.5，注记级/前沿未定。
- 智源 2026 十大趋势——§7.5，前沿未定（机构报告）。
- There Will Be a Scientific Theory of Deep Learning（arXiv:2604.21691）——全书总纲；全文见本地资料目录 references/（不入版本库）。

**第8章 理论之梯（经典与现代理论 → AI）**
- 信息论：Shannon（1948，熵/交叉熵）；MDL（Rissanen 1978）——8.0/8.1，定理/奠基。
- 算法信息论与归纳：Solomonoff（1964）、Kolmogorov（1963）——8.1，理论。
- 贝叶斯与统计决策：Bayes（1763）、Wald（1950）；无限宽↔高斯过程（Neal 1996）——8.1，主流。
- 随机逼近：Robbins & Monro（1951）——8.2，定理（奠基）。
- 动量与加速：Polyak（1964）、Nesterov（1983）→ Adam（Kingma & Ba 2015）——8.2，主流。
- 自然梯度与信息几何：Amari（1998）；SGLD（Welling & Teh 2011）——8.2，主流。
- 低秩参数化/内在维度：Aghajanyan 等（2020）——8.2，主流假说。
- 群论/等变：CNN（LeCun 1989）；Group-Equivariant CNN（Cohen & Welling 2016）——8.3，主流假说。
- 谱论/随机矩阵/低秩：NTK（Jacot 2018，§5 复用）；LoRA（Hu 等 2021）——8.3，主流。
- 压缩感知与稀疏恢复：Donoho（2006）、Candès 等（2006）——8.3，主流（§6 复用）。
- 傅里叶/散射：Mallat（2012）——8.3，主流。
- 自动机与形式语言：Turing（1936）、Chomsky（1956）——8.4。
- 通用逼近：Cybenko（1989）、Hornik（1991）——8.4，正向结果（表达）。
- 矩阵乘法复杂度：Strassen（1969）→ AlphaTensor（Fawzi 等，Nature 2022）——8.4/8.10，定理/前沿。
- 统计学习：Vapnik-Chervonenkis（1971）、Valiant（1984，PAC）——8.5，定理（§1.3/§7.3 复用）。
- 心理测量：因子分析（Spearman 1904）、项目反应理论 IRT（Rasch 1960）——8.5，主流（§7.1/§7.2 谱系源头）。
- PAC-Bayes：McAllester（1998）；均匀稳定性（Bousquet & Elisseeff 2002）——8.5，主流。
- 良性过拟合：Bartlett 等（2020）；平滑分析（Spielman & Teng 2009）——8.5，主流。
- 动态规划/MDP：Bellman（1957）、Howard（1960）→ DQN（Mnih 2015）——8.6。
- 最优控制：Pontryagin（1956）；策略梯度来源（Williams 1992）——8.6。
- POMDP：Kaelbling 等（1998）——8.6（《用好AI》第 2 章复用）。
- 博弈论：von Neumann（1928）、Nash（1950）→ AlphaZero（Silver 等 2017/18）——8.6。
- 逆强化学习/偏好学习：Ng & Russell（2000）→ RLHF（Christiano 等 2017）——8.6（《基座模型》第 3 章复用）。
- 机制设计/社会选择：Arrow（1951）、Maskin（1977）——8.6，主流。
- 因果：Pearl（2000/09）do 演算；IRM（Arjovsky 等 2019）；因果表示学习（Schölkopf 等 2021）——8.7。
- 相变/统计力学：mean-field（1940s 起）；能量模型 Hopfield（1982）→ 现代 Hopfield=attention（Ramsauer 等 2020）——8.8。
- 变分自由能/ELBO：Feynman（1972，泛函积分）、Neal & Hinton（1998）——8.8（《扩散》支流）。
- 信息瓶颈（Tishby 1999）——8.10，主流假说（表示压缩的源头）。
- 纠错码/鲁棒性（Hamming 1950）——8.4，主流。
- 认知/神经：双系统（Stanovich & West 1990s；Kahneman 2011 通俗化）；工作记忆（Baddeley 1974）；经验回放（Lin 1992；Mnih 2015）；预测编码（Rao & Ballard 1999）；稀疏编码（Olshausen & Field 1996）；反向传播（Rumelhart 1986）——8.9。
- 图灵完备 / 无 CoT 限 TC$^0$ / 不可幻觉无免费午餐 / 内对齐不可判定 / DNN 验证 NP-hard——见 §7 清单（§7.3 复用；本章 8.4 取正面读法）。
- 现代理论的辩证作用（综合 §3.4 涌现度量、§4.5 尺度断点、§5.2 NTK 边界、§6.5 SAE 边界、§7.1/§7.6 测量与方法论）——8.11，综合立场（共识/前沿未定）。
...
4. embedding-hole：$\mu$P 的超参迁移在 embedding 层失效——token 输入是一维稀疏向量，梯度统计与连续层不同，embedding 学习率需单独放大（Kalra 和 Barkeshli 等，arXiv:2605.21486；前沿未定：单篇预印本、新发现、可能被修正）。适用边界：参数化理论在同质连续特征层严格成立，异构结构（embedding/attention/MoE/归一化）要逐层核对。（§5.4）
